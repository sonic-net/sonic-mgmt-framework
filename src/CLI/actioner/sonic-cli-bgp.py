...
def invoke_api(func, args=[]):
    ...
    if func == 'patch_openconfig_network_instance_network_instances_network_instance_protocols_protocol_bgp_global_config_as':
        ...
        return api.patch(keypath, body), None
        ...
        body = { "openconfig-bgp-ext:origin": True if 'origin' in args[3:] else False }
        return api.patch(keypath, body), None
    elif func == 'get_openconfig_network_instance_network_instances_network_instance_protocols_protocol_bgp_rib_afi_safis_afi_safi_ipv4_unicast_neighbors_neighbor_adj_rib_in_pre':
        ...
        if op == 'patch':
            ...
            return api.delete(keypath), None
...
def run(func, args):
    response, responseSub = invoke_api(func, args)
    if response.ok():
        if responseSub is not None:
           resp1 = response.content
           resp2 = responseSub.content
           mrgdResponse = {key: value for (key, value) in (resp1.items() + resp2.items())}
           #print(mrgdResponse)
           show_cli_output(args[3], mrgdResponse)
           return 
